...
public class ObjectBuilder<X>
{
   ...
   public String replace(final String url, final X obj, final JsonObject source)
   {
      ...
      while (newUrl.indexOf("<") > 0)
      {
         ...
         try
         {
            Method m = baseClass.getMethod("get" + Character.toUpperCase(parameter.charAt(0))
                  + parameter.substring(1), new Class[0]);
            rep = m.invoke(obj, new Object[0]).toString();
         }
         catch (Exception e)
         {
            e.printStackTrace();
    ...
    */
   private JsonElement applyAnnotationMethod(final Method m, final LinkURL anno, final X object,
      ...
      try
      {
         ...
         if (pag == null)
         {
            ...
            if (anno.url().length() > 0)
            {
               ...
               SimpleCollectionUnbounded scu = new SimpleCollectionUnbounded<>(clz, service,
                     replace(anno.url(), object, source), anno.path());
            ...
            else
            {
               ...
               if (elem != null)
               {
                  ...
                  ObjectBuilder builder = ObjectBuilderFactory.getInstance().getObjectBuilder(
                        colClz, service);
         ...
         {
            WebList<?> wl = new WebList<>(clz, service, replace(anno.url(), object, source),
                  anno.path(), pag);
            m.invoke(object, new Object[]
            { wl });
         }
