...
public class ObjectBuilder<X> {
	 ...
	 */
	private JsonElement applyAnnotationMethod(final Method m,
			final LinkURL anno, final X object, final JsonObject source) {
	 ...
	 *            The json source
	 */
	private void applyCollectionMethod(final Method m, final X object,
	 ...
	 *            The json source
	 */
	private void applyMethod(final Method m, final X object,
			final JsonObject source) {
		try {
	 ...
	 */
	private Object[] buildParams(final Method m, final JsonObject source,
			final String[] pNames) {
		Object[] toRet = new Object[pNames.length];
		for (int i = 0; i < toRet.length; i++) {
			Class<?> type = m.getParameterTypes()[i];
	 ...
	 * @return The converted object
	 */
	private Object convert(final JsonElement source, final String paramName,
			final Class<?> clz) {
